...
def handleDecodeFile(file_data,is_image,is_video):
    ...
    if is_image is True:
        ...
        with open( os.path.join(path_upload_image, "comments" , name_file), 'wb+') as decoded_image_file:
            decoded_image_file.write(file_bytes)
            return { 
                'name' : name_file ,
                'file' : 'media/photos/comments/' + name_file ,
                'type' : False
            }
    if is_video is True:
        name_file = 'blog' + str(uuid.uuid4()) + '.' + file_ext
        with open( os.path.join(path_upload_video, "comments",name_file), 'wb+') as decoded_image_file:
            decoded_image_file.write(file_bytes)
            return {
                'name' : name_file,
                'file' : 'media/videos/comments/' + name_file ,
                'type' : True
            }
